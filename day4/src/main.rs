...
fn main() {
    ...
    let fully_contained_count = input.clone().map(|line| {
    ...
    let partially_contained_count = input.map(|line| {
        let mut parts = line.split(&['-', ',']).map(|n| n.parse::<i32>().unwrap());
        let elf1_start = parts.next().unwrap();
        let elf1_end = parts.next().unwrap();
        let elf2_start = parts.next().unwrap();
    ...
    println!("number of partially contained: {}", partially_contained_count);
}
