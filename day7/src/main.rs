...
fn main() {
    ...
    for line in input {
        regex.captures(line).map(|captures| {
            ...
            } else { // if none of the others match, it's the listing of a file
    ...
    let directory_sizes : HashMap<String, i32> = all_dirs.clone().into_iter().map(|dir| {
        let dir_size: i32 = files.iter().filter(|(file, _)| file.starts_with(&(dir.to_string() + "/"))).map(|(_, size)| size).sum();
        (dir, dir_size)
    }).collect();

    let sum_of_directories_smaller_than_100k : i32 = directory_sizes.clone().into_iter().filter(|(_, size)| size <= &100_000).map(|(_, size)| size).sum();

    println!("sum of directories smaller than 100k: {}", sum_of_directories_smaller_than_100k);
    ...
    let smallest_directory_to_delete : i32 = directory_sizes.into_iter().map(|(_, size)| size).filter(|size| size >= &needed_space).min().unwrap();

    println!("smallest directory to delete to make enough space: {}", smallest_directory_to_delete);
}
