...
fn main() {
    ...
    let sum_of_priorities : u32 = input.clone().map(|line| {
    ...
    let sum_of_badges : u32 = vec_input.chunks(3).map(|arr| {
        ...
        let first_combination : HashSet<char> = HashSet::from_iter(first_part.intersection(&second_part).map(|c| *c));
        first_combination.intersection(&third_part).into_iter().next().unwrap().clone()
    }).map(char_value).sum();
